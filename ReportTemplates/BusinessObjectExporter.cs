...
using System.Linq;
using System.Text;
using RuiguCrmReports.AfterSale;
using RuiguCrmReports.DeliveryTask;
...
namespace RuiguCrmReports
{
    public class BusinessObjectExporter
    {
        ...
        public static string ExportWabillTask(int dcId, string warehouseName, string savingPath)
        {
            savingPath = GetFileSavingPath(savingPath);
            string fileName = string.Format("{0}\\{1}{2}{3}.pdf", savingPath, warehouseName, "分拨表",DateTime.Now.ToString("yy-MM-dd"));
            WaybillTaskList pp = new WaybillTaskList();
            if (!pp.SetDateSource(dcId))
                return string.Empty;
