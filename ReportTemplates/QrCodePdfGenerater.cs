...
using System.Data;
using System.IO;
using RuiguCrmReports.Supplier;
...
namespace RuiguCrmReports
{
    public class QrCodePdfGenerater
    {
        ...
        public static Exception GeneratePdfWithDataTableAndFileName(DataTable workingDataTable, string savingFileFullName, LabelSetting setting)
        {
            ...
            try
            {
                if (CheckExcelFormat(workingDataTable))
                {
                    if (workingDataTable.Rows.Count > MaxRowsForPdf)
                    {
                        ...
                        FileUtility.CreateDir(childFolder);
                        ...
                        for (int i = 0; i < tableCount; i++)
                        {
                            ...
                            exportException = CreatePdfFileWithDataTable(splitedTable, fileName, setting);
                    ...
                    {
                        exportException = CreatePdfFileWithDataTable(workingDataTable, savingFileFullName, setting);
                    }
                }
            }
            catch
        ...
        private static Exception CreatePdfFileWithDataTable(DataTable excelDataTable, string pdfFileName, LabelSetting setting)
        {
            //选择产品
            Exception exportException = null;
